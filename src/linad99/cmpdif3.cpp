...
void verify_identifier_string(const char* str1)
{
  ...
  char str[10];
  str[num_bytes]='\0';
  gradient_structure::get()->get_fp()->fread(str,num_bytes);
...
adstring get_string_marker(void)
{
  ...
  assert(num_bytes > 0);
#endif
  gradient_structure::get()->get_fp()->fread(str,(size_t)num_bytes);
  //clogf << "in verify_id_string " << str1 << endl;
  str1=str;
#endif
 return str1;
...
void save_ad_pointer(void * p)
{
  ...
  gradient_structure::get()->get_fp()->fwrite(&p, wsize);
}
...
void * restore_ad_pointer(void)
{
  ...
  // saves the size and address information for a dvar_vector
  size_t wsize=sizeof(void *);
  gradient_structure::get()->get_fp()->fread(&p, wsize);
  return p;
}
