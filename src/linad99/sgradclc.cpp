...
void gradcalc(int nvar, const dvector& _g)
{
  ...
  if (nvar!=0)
  {
    int NVAR = gs ? static_cast<int>(gs->INDVAR_LIST.get_nvar()) : 0;
    if (nvar != NVAR)
    {
      cerr << "nvar != gradient_structure::NVAR in gradcalc" << endl;
      cerr << "  nvar = " << nvar << endl;
      cerr << "  gradient_structure::NVAR = " << NVAR << endl;
...
#if defined(DOS386)
  #ifdef OPT_LIB
     write(GRAD_STACK1->_VARSSAV_PTR,
       (char*)src, bytes_needed);
  #else
     ssize_t ret = write(GRAD_STACK1->_VARSSAV_PTR,
       (char*)src, bytes_needed);
     assert(ret != -1);
  #endif
#else
